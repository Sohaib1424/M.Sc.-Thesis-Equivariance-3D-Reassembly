...
def get_features(
    ...
    # writing into those would be undefined rather than merely noisy, so one
    # fragment's worth of vertices is copied -- a negligible cost.
    ...
    node_vec = np.stack([pos, normals], axis=1)     # (V, 2, 3)

    if vertex_cluster_ids is None:
        v_clusters = np.full(num_v, -1, np.int64)
    else:
        v_clusters = np.asarray(vertex_cluster_ids, dtype=np.int64)
    ...
    if num_e == 0:
        empty = FragmentGraph(
            ...
            edge_len=np.zeros((0, 1), np.float32),
            edge_vec=np.zeros((0, 3, 3), np.float32),
            centroid=centroid.astype(np.float32),
            vertex_cluster_id=v_clusters,
            edge_cluster_id=np.zeros(0, np.int64),
        )
    ...
    edge_vec = np.stack([edge_mid, n1, n2], axis=1)  # (E, 3, 3)

    if edge_cluster_ids is None:
        e_clusters = np.full(num_e, -1, np.int64)
    else:
        e_clusters = np.asarray(edge_cluster_ids, dtype=np.int64)

    graph = FragmentGraph(
        node_vec=node_vec,
        edge_index=np.ascontiguousarray(edges_unique.astype(np.int64).T),
        edge_len=edge_len,
        edge_vec=edge_vec,
        centroid=centroid.astype(np.float32),
        vertex_cluster_id=v_clusters,
