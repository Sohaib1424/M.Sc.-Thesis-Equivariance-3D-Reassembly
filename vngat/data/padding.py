...
class PaddedBatch:
    ...
    graph: SceneBatch
    node_mask: np.ndarray      # (N_pad,) True for real vertices
    edge_mask: np.ndarray      # (E_pad,)
    frag_mask: np.ndarray      # (F_pad,)
    real_nodes: int
    real_edges: int
    real_fragments: int
...
def pad_scene_batch(
    ...
    def grow(t: np.ndarray, target: int, fill: float = 0.0) -> np.ndarray:
        if t.shape[0] == target:
            return t
        shape = (target - t.shape[0], *t.shape[1:])
        return np.concatenate([t, np.full(shape, fill, dtype=t.dtype)], 0)
    ...
    if e_pad > e:
        filler = np.full((2, e_pad - e), pad_vertex, dtype=edge_index.dtype)
        edge_index = np.concatenate([edge_index, filler], axis=1)
    ...
    idx_n, idx_e, idx_f = np.arange(n_pad), np.arange(e_pad), np.arange(f_pad)
    return PaddedBatch(
        graph=padded,
