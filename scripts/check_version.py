...
MARKERS = [
    ...
    ("scaling sweep controls per-object budget",
     "scripts/scaling_sweep.py", "steps_per_object"),
    ("lr warmup",
     "vngat/config.py", "lr_warmup_epochs"),
    ("fp32 attention scores (fp16 overflow fix)",
     "vngat/models/gat_layer.py", "at_least_float32(q_i)"),
    ("fp32 scores in the virtual-node block",
     "vngat/models/virtual_nodes.py", "at_least_float32(q_node)"),
    ("softmax bounded against inf",
     "vngat/models/segment_ops.py", "work.clamp(-finite_max, finite_max)"),
    ("per-scene NaN tally",
     "vngat/training/trainer.py", "nan_scenes"),
    ("scene NaN diagnostic script",
    ...
    ("non-finite feature repair",
     "vngat/data/features.py", "def _sanitise"),
    ("sweep detects truncation",
     "scripts/scaling_sweep.py", '"truncated":'),
    ...
    ("static-shape padding for XLA",
     "vngat/data/padding.py", "def pad_scene_batch"),
    ("mask-aware loss reductions",
     "vngat/losses/composite.py", "def _reduce"),
    ("TPU entry point",
     "scripts/train_tpu.py", "xmp.spawn"),
    ("padding equivalence tests",
     "tests/test_padding.py", "test_padded_loss_equals_unpadded_loss"),
    ("schedule restart on resume",
     "vngat/config.py", "restart_schedule"),
    ("tilt/twist reported during training",
     "vngat/losses/composite.py", '"tilt": _reduce(tilt.detach()'),
    ("deterministic single-fracture loader",
     "vngat/data/io.py", "def load_scene"),
    ("prediction dump script",
     "scripts/dump_prediction.py", "vertex_offsets"),
    ("cosine anneals once, does not cycle",
     "vngat/training/trainer.py", "phase = min(epoch, t_max)"),
    ("scheduler state guarded by type",
     "vngat/training/checkpoint.py", "scheduler_type"),
    ("split lists found when nested",
     "vngat/data/splits.py", 'split_dir.rglob(f"*.{split}.txt")'),
]
